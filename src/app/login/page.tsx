...
export default function LoginPage() {
  ...
  return (
    <main className="force-light w-full flex flex-col bg-slate-50">
      {/* ── Login Hero ── */}
      <div className="flex min-h-screen">
        {/* Left panel */}
        <div className="hidden lg:flex lg:w-[45%] relative overflow-hidden flex-col justify-center gap-10 p-12" style={{ background: "linear-gradient(135deg, #4F46E5 0%, #3730A3 100%)" }}>
          ...
          <div className="relative z-10 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center"><span className="text-white font-black text-lg">N</span></div>
          ...
          <div className="relative z-10 flex gap-8">
            {[{ value: "<1s", label: "QR Scan Time" }, { value: "100%", label: "Accuracy" }, { value: "Live", label: "Notifications" }].map((s) => (
              <div key={s.label}><p className="text-white font-bold text-2xl">{s.value}</p><p className="text-white/50 text-sm">{s.label}</p></div>
            ))}
          </div>
          {/* Scroll hint — desktop */}
          <motion.button
            onClick={() => window.scrollBy({ top: window.innerHeight, behavior: "smooth" })}
            className="relative z-10 self-start flex items-center gap-2.5 px-4 py-2.5 rounded-full border border-white/25 bg-white/10 hover:bg-white/20 hover:border-white/40 transition-all duration-200 cursor-pointer group"
            initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2 }}
          >
            <span className="text-white/80 text-sm font-medium group-hover:text-white transition-colors">Explore features</span>
            <motion.div
              animate={{ y: [0, 4, 0] }}
              transition={{ duration: 1.4, repeat: Infinity, ease: "easeInOut" }}
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={2.5} className="w-4 h-4 opacity-80 group-hover:opacity-100 transition-opacity">
        ...
        <div className="flex-1 flex flex-col lg:items-center lg:justify-center">
          ...
          <div className="lg:hidden relative overflow-hidden px-6 pt-12 pb-10 flex-shrink-0" style={{ background: "linear-gradient(135deg, #4F46E5 0%, #3730A3 100%)" }}>
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2.5 mb-6">
                <div className="w-9 h-9 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center"><span className="text-white font-black text-sm">N</span></div>
              ...
              <div className="flex gap-6 mt-5">
                {[{ value: "<1s", label: "Scan" }, { value: "100%", label: "Accuracy" }, { value: "Live", label: "Alerts" }].map((s) => (
                  <div key={s.label}><p className="text-white font-bold text-lg">{s.value}</p><p className="text-white/40 text-[10px] font-medium uppercase tracking-wider">{s.label}</p></div>
                ))}
              </div>
            </div>
          </div>
          ...
          {/* Form */}
          <div className="flex-1 flex items-start lg:items-center justify-center px-5 sm:px-6 pt-5 pb-4 lg:py-0">
                ...
                <AnimatePresence mode="wait">
                  {step === "login" && (
                    <motion.div key="login" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}>
                      <div className="mb-8"><h1 className="text-2xl font-bold text-slate-800">Welcome back</h1><p className="text-slate-400 text-base lg:text-sm mt-1">Sign in to your Nexora account</p></div>
                  ...
                  {step === "forgotEmail" && (
                    <motion.div key="forgotEmail" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}>
                      <div className="mb-8"><h1 className="text-2xl font-bold text-slate-800">Forgot password?</h1><p className="text-slate-400 text-base lg:text-sm mt-1">Enter your email and we&apos;ll send you a verification code.</p></div>
          ...
          <motion.button
            onClick={() => window.scrollBy({ top: window.innerHeight, behavior: "smooth" })}
            className="lg:hidden flex items-center gap-2 px-4 py-2.5 mx-auto mb-6 rounded-full border border-slate-200 bg-white shadow-sm hover:shadow-md hover:border-indigo-200 hover:bg-indigo-50 transition-all duration-200 cursor-pointer group"
            initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}
          >
            <span className="text-slate-500 text-sm font-medium group-hover:text-indigo-600 transition-colors">Explore features</span>
            <motion.div
              animate={{ y: [0, 4, 0] }}
              transition={{ duration: 1.4, repeat: Infinity, ease: "easeInOut" }}
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} className="w-4 h-4 text-slate-400 group-hover:text-indigo-500 transition-colors">
                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
              </svg>
            </motion.div>
          </motion.button>
