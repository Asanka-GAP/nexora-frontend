...
import { getInstituteClasses, createInstituteClass, updateInstituteClass, deleteInstituteClass, toggleInstituteClassStatus, addInstituteClassSchedule, deleteInstituteClassSchedule, getInstituteTeachers, getClassrooms, getAvailableClassrooms } from "@/services/api";
import { DAYS } from "@/lib/utils";
import type { InstituteClassResponse, InstituteTeacherResponse, ClassroomResponse } from "@/lib/types";
...
const emptyForm = { name: "", grade: 1, subject: "", location: "", instituteTeacherId: "", classroomId: "" };
const PAGE_SIZE = 10;
...
export default function InstituteClassesPage() {
  ...
  const [editingClass, setEditingClass] = useState<InstituteClassResponse | null>(null);
  const [form, setForm] = useState(emptyForm);
  const [schedules, setSchedules] = useState<ScheduleEntry[]>([]);
  const [saving, setSaving] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<InstituteClassResponse | null>(null);
  const [deleting, setDeleting] = useState(false);
  const [scheduleClass, setScheduleClass] = useState<InstituteClassResponse | null>(null);
  ...
  const [availableRooms, setAvailableRooms] = useState<ClassroomResponse[]>([]);
  ...
  const goSchedPage = (p: number) => { setSchedDirection(p > schedPage ? 1 : -1); setSchedPage(p); };

  const load = useCallback(() => {
    setLoading(true);
    Promise.all([
      getInstituteClasses().then(setClasses),
      getInstituteTeachers().then(setTeachers),
  ...
  const filtered = all.filter(c => {
    ...
    return c.name.toLowerCase().includes(q) || (c.subject?.toLowerCase().includes(q) ?? false) || (c.teacherName?.toLowerCase().includes(q) ?? false);
  });
  ...
  const handleToggle = async (cls: InstituteClassResponse) => {
    setClasses(prev => prev.map(c => c.id === cls.id ? { ...c, isActive: !c.isActive } : c));
    try { await toggleInstituteClassStatus(cls.id); toast.success(cls.isActive ? `${cls.name} deactivated` : `${cls.name} activated`); }
    catch { toast.error("Failed to update status"); setClasses(prev => prev.map(c => c.id === cls.id ? { ...c, isActive: cls.isActive } : c)); }
  };

  const openCreate = () => { setEditingClass(null); setForm(emptyForm); setSchedules([]); setAvailableRooms([]); setModalOpen(true); };
  const openEdit = (cls: InstituteClassResponse) => { setEditingClass(cls); setForm({ name: cls.name, grade: cls.grade, subject: cls.subject || "", location: "", instituteTeacherId: cls.instituteTeacherId || "", classroomId: cls.classroomId || "" }); setSchedules([]); setAvailableRooms([]); setModalOpen(true); };

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name) { toast.error("Class name is required"); return; }
    if (!editingClass && !form.instituteTeacherId) { toast.error("Teacher is required"); return; }
    if (!editingClass && !form.subject) { toast.error("Subject is required"); return; }
    setSaving(true);
    try {
      if (editingClass) {
        await updateInstituteClass(editingClass.id, {
          name: form.name,
          grade: Number(form.grade),
          subject: form.subject.trim() || undefined,
          instituteTeacherId: form.instituteTeacherId || undefined,
          classroomId: form.classroomId || undefined,
        });
        toast.success("Class updated");
      } else {
        await createInstituteClass({
          name: form.name,
          grade: Number(form.grade),
          subject: form.subject.trim() || undefined,
          instituteTeacherId: form.instituteTeacherId || undefined,
          ...
          schedules: schedules.length > 0 ? schedules.map(s => ({
            ...
            dayOfWeek: s.scheduleType === "WEEKLY" ? s.dayOfWeek : undefined,
            sessionDate: s.scheduleType === "ONE_TIME" ? s.sessionDate : undefined,
      ...
      setModalOpen(false); setForm(emptyForm); setSchedules([]); setEditingClass(null); load();
    } catch (err: any) { toast.error(editingClass ? "Failed to update class" : err?.response?.data?.message || "Failed to create class"); }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try { await deleteInstituteClass(deleteTarget.id); toast.success(`${deleteTarget.name} deleted`); setDeleteTarget(null); load(); }
  ...
  const handleAddSchedule = async () => {
    ...
    setAddingSched(true);
    try {
      await addInstituteClassSchedule(scheduleClass.id, { scheduleType: newSchedule.scheduleType, dayOfWeek: newSchedule.scheduleType === "WEEKLY" ? newSchedule.dayOfWeek : undefined, sessionDate: newSchedule.scheduleType === "ONE_TIME" ? newSchedule.sessionDate : undefined, startTime: newSchedule.startTime, endTime: newSchedule.endTime });
      toast.success("Schedule added");
      setNewSchedule({ scheduleType: "WEEKLY", dayOfWeek: 1, sessionDate: "", startTime: "08:00", endTime: "09:00" });
      load();
      const updated = (await getInstituteClasses()).find(c => c.id === scheduleClass.id);
      if (updated) { setScheduleClass(updated); const lastPage = Math.ceil((updated.schedules?.length ?? 0) / SCHED_PAGE_SIZE); setSchedDirection(1); setSchedPage(lastPage); }
  ...
  const handleDeleteSchedule = async (scheduleId: string) => {
    if (!scheduleClass) return;
    try {
      await deleteInstituteClassSchedule(scheduleClass.id, scheduleId);
      toast.success("Schedule removed");
      load();
      const newScheds = scheduleClass.schedules.filter(s => s.id !== scheduleId);
      setScheduleClass({ ...scheduleClass, schedules: newScheds });
      const newTotal = Math.ceil(newScheds.length / SCHED_PAGE_SIZE);
  ...
  const addScheduleRow = () => {
    const newRow: ScheduleEntry = { scheduleType: "WEEKLY", dayOfWeek: 1, sessionDate: "", startTime: "08:00", endTime: "09:00" };
    setSchedules([newRow]);
    fetchAvailableRooms(newRow);
    setForm(f => ({ ...f, classroomId: "" }));
  };
  const removeScheduleRow = (idx: number) => { setSchedules(schedules.filter((_, i) => i !== idx)); setAvailableRooms([]); setForm(f => ({ ...f, classroomId: "" })); };
  const updateScheduleRow = (idx: number, key: keyof ScheduleEntry, val: string | number) => {
    const updated = schedules.map((s, i) => i === idx ? { ...s, [key]: val } : s);
    setSchedules(updated);
    const s = updated[0];
    if (s) fetchAvailableRooms(s);
    setForm(f => ({ ...f, classroomId: "" }));
  };

  const fetchAvailableRooms = async (s: ScheduleEntry) => {
    if (!s.startTime || !s.endTime) return;
    if (s.scheduleType === "ONE_TIME" && !s.sessionDate) return;
    ...
    } catch { setAvailableRooms(rooms.filter(r => r.isActive)); }
    finally { setCheckingRooms(false); }
  };

  const parse24 = (t: string) => { const [h, m] = t.split(":").map(Number); return { h12: h === 0 ? 12 : h > 12 ? h - 12 : h, min: m, ampm: h < 12 ? "AM" as const : "PM" as const }; };
  const to24 = (h12: number, min: number, ampm: "AM" | "PM") => { const h = ampm === "AM" ? (h12 === 12 ? 0 : h12) : (h12 === 12 ? 12 : h12 + 12); return `${String(h).padStart(2, "0")}:${String(min).padStart(2, "0")}`; };
  ...
  const TimeInput = ({ value, min, max, pad, onCommit }: { value: number; min: number; max: number; pad?: boolean; onCommit: (n: number) => void }) => {
    ...
    return (
      <input ref={ref} type="text" inputMode="numeric" maxLength={2} value={draft}
        onChange={e => { const v = e.target.value.replace(/\D/g, "").slice(0, 2); if (v === "") { setDraft(""); return; } const n = parseInt(v, 10); if (n > max) return; setDraft(v); }}
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        <div className="flex flex-col gap-0.5 ml-1">
          <button type="button" onClick={() => onChange(to24(p.h12, p.min, "AM"))} className={`px-2 py-1 rounded-lg text-[10px] font-bold transition-all ${p.ampm === "AM" ? "bg-blue-700 text-white shadow-sm" : "bg-bg border border-border text-text-muted hover:border-blue-600"}`}>AM</button>
          <button type="button" onClick={() => onChange(to24(p.h12, p.min, "PM"))} className={`px-2 py-1 rounded-lg text-[10px] font-bold transition-all ${p.ampm === "PM" ? "bg-blue-700 text-white shadow-sm" : "bg-bg border border-border text-text-muted hover:border-blue-600"}`}>PM</button>
        </div>
      </div>
    );
  };

  if (loading && !classes.length) return <PageSkeleton />;
    ...
    <>
      ...
      <div className="bg-bg-card rounded-2xl shadow-sm border border-border p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, subject, teacher..." className="w-full pl-10 pr-4 py-2.5 border border-border rounded-xl text-sm text-text bg-bg-card placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
          </div>
          <div className="flex items-center bg-bg rounded-xl p-0.5 border border-border flex-shrink-0">
            {(["ALL", "ACTIVE", "INACTIVE"] as const).map(s => (
              <button key={s} onClick={() => setFilterStatus(s)} className={`px-3 py-1.5 rounded-[10px] text-[11px] font-semibold transition-all whitespace-nowrap ${filterStatus === s ? "bg-bg-card text-text shadow-sm" : "text-text-muted hover:text-text"}`}>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  {["Class", "Grade", "Teacher", "Sessions", "Students", "Status", ""].map(h => (
                  ...
                  <motion.tr key={cls.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0"><BookOpen className="w-4 h-4 text-primary" /></div>
                        <div className="min-w-0">
                          <span className="text-sm font-semibold text-text">{cls.name}</span>
                          {cls.subject && <p className="text-xs text-text-muted truncate max-w-[180px]">{cls.subject}</p>}
                          {cls.classroomName && <p className="text-xs text-text-muted truncate max-w-[180px]"><MapPin className="w-3 h-3 inline mr-0.5" />{cls.classroomName}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4"><span className="text-xs font-semibold px-2.5 py-1 rounded-lg bg-secondary/15 text-cyan-800">{cls.grade}</span></td>
                    <td className="px-5 py-4">
                      {cls.teacherName ? <span className="text-sm text-text">{cls.teacherName}</span> : <span className="text-xs text-text-muted">—</span>}
                    </td>
                    <td className="px-5 py-4">
                      {(() => {
                        const comp = cls.completedSessions || 0; const canc = cls.cancelledSessions || 0; const past = comp + canc;
                        ...
                        return (
                          <div className="min-w-[100px]">
                            <div className="flex items-center justify-between mb-1"><span className="text-xs font-semibold text-text">{past > 0 ? `${pct}%` : "—"}</span><span className={`text-[10px] font-semibold ${statusColor}`}>{statusLabel}</span></div>
                            <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-500 ${barColor}`} style={{ width: `${past > 0 ? pct : 0}%` }} /></div>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        <button onClick={() => handleToggle(cls)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${cls.isActive ? "bg-emerald-500" : "bg-red-500"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div>
                        ...
                        {cls.subject && <p className="text-xs text-text-muted">{cls.subject}</p>}
                        {cls.teacherName && <p className="text-xs text-text-muted">{cls.teacherName}</p>}
                      </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleToggle(cls)} className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${cls.isActive ? "bg-emerald-500" : "bg-red-500"}`}>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="mb-3">
                          <div className="flex items-center justify-between mb-1"><span className="text-xs text-text-muted"><span className="font-bold text-text">{past > 0 ? `${pct}%` : "—"}</span> completion</span><span className={`text-[10px] font-semibold ${statusColor}`}>{statusLabel}</span></div>
                          <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-500 ${barColor}`} style={{ width: `${past > 0 ? pct : 0}%` }} /></div>
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setSchedules([]); setEditingClass(null); }} title="" className="max-w-xl p-0 !rounded-none !border-0">
        <form onSubmit={handleSave}>
          <div className="bg-gradient-to-r from-blue-700 to-blue-800 px-6 py-5">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-white/20 flex items-center justify-center">
                {editingClass ? <span className="text-white font-bold text-lg">{editingClass.name.charAt(0).toUpperCase()}</span> : <BookOpen className="w-5 h-5 text-white" />}
              </div>
              <div>
          ...
          <div className="px-6 py-5 space-y-5 max-h-[60vh] overflow-y-auto">
            <div>
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-text-muted mb-1.5">Class Name</label>
                    <div className="relative"><BookOpen className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted/50 pointer-events-none" />
                      <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Math 101" className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-border bg-bg-card text-sm text-text placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative"><GraduationCap className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted/50 pointer-events-none" />
                      <input type="number" value={String(form.grade)} onChange={e => setForm({ ...form, grade: Number(e.target.value) })} className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-border bg-bg-card text-sm text-text focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-text-muted mb-1.5">Teacher <span className="text-danger">*</span></label>
                    <select value={form.instituteTeacherId} onChange={e => setForm({ ...form, instituteTeacherId: e.target.value })} className="w-full px-4 py-2.5 rounded-xl border border-border bg-bg-card text-sm text-text focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                      <option value="">Select teacher...</option>
                      {teachers.map(t => <option key={t.id} value={t.id}>{t.name}{t.subject ? ` · ${t.subject}` : ""}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-text-muted mb-1.5">Subject <span className="text-danger">*</span></label>
                    <input value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} placeholder="e.g. Mathematics" className="w-full px-4 py-2.5 rounded-xl border border-border bg-bg-card text-sm text-text placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                  </div>
                </div>
              </div>
            </div>
            {!editingClass && (
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  {schedules.length === 0 && <button type="button" onClick={addScheduleRow} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-sm hover:shadow transition-all"><Plus className="w-3 h-3" /> Add Slot</button>}
                </div>
                {schedules.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-6 rounded-xl border border-dashed border-border">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center mb-2"><Clock className="w-5 h-5 text-primary" /></div>
                  ...
                  <div className="space-y-3">
                    {schedules.map((s, idx) => (
                      <div key={idx} className="rounded-xl border border-border bg-bg-card p-4">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center"><span className="text-[10px] font-bold text-white">{idx + 1}</span></div>
                            <p className="text-xs font-semibold text-text">Time Slot {idx + 1}</p>
                          </div>
                          <button type="button" onClick={() => removeScheduleRow(idx)} className="p-1.5 rounded-lg text-text-muted/40 hover:text-danger hover:bg-danger/10 transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                        <div className="mb-3">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {DAYS.map((d, i) => (
                                <button key={i} type="button" onClick={() => updateScheduleRow(idx, "dayOfWeek", i)}
                                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${s.dayOfWeek === i ? "bg-blue-700 text-white shadow-sm" : "bg-bg-card text-text-muted hover:bg-bg"}`}>{d.slice(0, 3)}</button>
                              ))}
                            </div>
                          </div>
                        ) : (
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-3 justify-center">
                          <div className="text-center"><label className="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-2">Start</label><TimePicker value={s.startTime} onChange={v => updateScheduleRow(idx, "startTime", v)} /></div>
                          <div className="pt-5 hidden sm:block"><div className="w-4 h-px bg-border" /></div>
                          <div className="text-center"><label className="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-2">End</label><TimePicker value={s.endTime} onChange={v => updateScheduleRow(idx, "endTime", v)} /></div>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {/* Classroom — shown after schedule is added, edit mode always shown */}
                ...
                <label className="block text-xs font-semibold text-text-muted mb-1.5">
                  ...
                  {!editingClass && schedules.length > 0 && (
                    <span className="ml-2 text-[10px] font-normal text-text-muted/60">
                      {checkingRooms ? "Checking availability..." : availableRooms.length === 0 ? "No rooms available at this time" : `${availableRooms.length} room${availableRooms.length !== 1 ? "s" : ""} available`}
                    </span>
                  )}
                </label>
                <select
                  ...
                  disabled={!editingClass && checkingRooms}
                  className="w-full px-4 py-2.5 rounded-xl border border-border bg-bg-card text-sm text-text focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary disabled:opacity-50">
                  <option value="">None</option>
                  {(editingClass ? rooms.filter(r => r.isActive) : availableRooms).map(r => (
                    <option key={r.id} value={r.id}>{r.name}{r.capacity ? ` (${r.capacity} seats)` : ""}{r.location ? ` · ${r.location}` : ""}</option>
                  ))}
                </select>
                {!editingClass && schedules.length > 0 && availableRooms.length === 0 && !checkingRooms && (
                  <p className="text-[11px] text-amber-600 mt-1.5 flex items-center gap-1">
                    <span>⚠</span> All classrooms are booked at this time. You can still create the class without a room.
                  </p>
                )}
              </div>
            )}
      ...
      <Modal open={!!scheduleClass} onClose={() => { setScheduleClass(null); setSchedPage(1); setSchedDirection(0); }} title="" className="max-w-lg p-0">
        <div className="bg-gradient-to-r from-sky-600 to-cyan-700 px-6 py-5 rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-white/20 flex items-center justify-center"><Clock className="w-5 h-5 text-white" /></div>
            <div>
              <h2 className="text-white font-semibold text-base">Schedule</h2>
              <p className="text-white/60 text-xs mt-0.5">{scheduleClass?.name}{scheduleClass?.classroomName ? ` · ${scheduleClass.classroomName}` : ""}</p>
            </div>
          </div>
        </div>
        <div className="px-6 py-5 space-y-5 max-h-[60vh] overflow-y-auto">
          <div>
            ...
            {schedList.length ? (
              <div>
                <div className="relative overflow-hidden">
                  <AnimatePresence mode="wait" initial={false} custom={schedDirection}>
                    <motion.div key={schedPage} custom={schedDirection}
                      variants={{ enter: (dir: number) => ({ x: dir > 0 ? 80 : -80, opacity: 0 }), center: { x: 0, opacity: 1 }, exit: (dir: number) => ({ x: dir > 0 ? -80 : 80, opacity: 0 }) }}
                      initial="enter" animate="center" exit="exit"
                      transition={{ type: "spring", stiffness: 400, damping: 35, mass: 0.8 }}
                      className="space-y-2">
                      {schedPaginated.map((s, i) => (
                        <motion.div key={s.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06, duration: 0.2 }}
                ...
                {schedTotalPages > 1 && (
                  <div className="flex items-center justify-between pt-3">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => goSchedPage(schedPage - 1)} disabled={schedPage === 1} className="px-2 py-1 text-xs rounded-lg text-text-muted hover:bg-border/50 disabled:opacity-30 transition">‹</button>
                      {Array.from({ length: schedTotalPages }, (_, i) => i + 1).map(p => (
                        <button key={p} onClick={() => goSchedPage(p)} className={`min-w-[24px] py-1 text-[11px] rounded-lg font-semibold transition ${schedPage === p ? "text-white bg-primary shadow-sm" : "text-text-muted hover:bg-bg"}`}>{p}</button>
                      ))}
                      <button onClick={() => goSchedPage(schedPage + 1)} disabled={schedPage === schedTotalPages} className="px-2 py-1 text-xs rounded-lg text-text-muted hover:bg-border/50 disabled:opacity-30 transition">›</button>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-8 rounded-xl border border-dashed border-border">
                <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center mb-2"><CalendarDays className="w-5 h-5 text-primary" /></div>
          ...
          <div>
            ...
            <div className="rounded-xl border border-border bg-bg-card p-4 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {DAYS.map((d, i) => (
                      <button key={i} type="button" onClick={() => setNewSchedule({ ...newSchedule, dayOfWeek: i })}
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${newSchedule.dayOfWeek === i ? "bg-primary text-white shadow-sm" : "bg-bg-card text-text-muted hover:bg-bg"}`}>{d.slice(0, 3)}</button>
                    ))}
                  </div>
                </div>
              ) : (
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 justify-center">
                <div className="text-center"><label className="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-2">Start</label><TimePicker value={newSchedule.startTime} onChange={v => setNewSchedule({ ...newSchedule, startTime: v })} /></div>
                <div className="pt-5 hidden sm:block"><div className="w-4 h-px bg-border" /></div>
                <div className="text-center"><label className="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-2">End</label><TimePicker value={newSchedule.endTime} onChange={v => setNewSchedule({ ...newSchedule, endTime: v })} /></div>
              </div>
            </div>
          </div>
        </div>
