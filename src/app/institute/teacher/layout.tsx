...
import { Sun, Moon, X, LogOut, LayoutDashboard, BookOpen, CheckSquare } from "lucide-react";
...
const navItems = [
  { href: "/institute/teacher/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { href: "/institute/teacher/classes", label: "My Classes", icon: BookOpen },
  { href: "/institute/teacher/attendance", label: "Attendance", icon: CheckSquare },
];

const pageMeta: Record<string, { title: string; subtitle: string }> = {
  "/institute/teacher/dashboard": { title: "Dashboard", subtitle: "Here's your teaching overview for today." },
  "/institute/teacher/classes": { title: "My Classes", subtitle: "Classes assigned to you." },
  "/institute/teacher/attendance": { title: "Attendance", subtitle: "Mark and view attendance records." },
};

export default function InstituteTeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading && (!user || user.role !== "INSTITUTE_TEACHER")) router.push("/login");
  }, [user, loading, router]);
  ...
  if (loading || !user || user.role !== "INSTITUTE_TEACHER") {
    return (
          ...
          <AnimatePresence>
            {expanded && (
              <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }}
                ...
                <p className="text-xs text-text-muted leading-tight">Institute Teacher</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          <div className="px-3 py-4 border-t border-border">
            <div className="flex items-center gap-3 px-2.5 py-2 mb-1">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-text-muted">{user.subject || "Teacher"}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between px-8 py-4">
              <div>
                <h2 className="text-xl font-bold text-text">
                  {pathname === "/institute/teacher/dashboard"
                  ...
                  {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-bg-card rounded-xl shadow-lg border border-border py-2 z-50">
                      <div className="px-4 py-2.5 border-b border-border">
                        <p className="text-sm font-semibold text-text truncate">{userName}</p>
                        <p className="text-xs text-text-muted">{user.subject || "Institute Teacher"}</p>
                      </div>
