...
export default function InstituteCalendarPage() {
    ...
    const [selectedEvent, setSelectedEvent] = useState<InstituteSessionResponse | null>(null);
    const [selectedDate, setSelectedDate] = useState(toStr(new Date()));
    const [actionNote, setActionNote] = useState("");
    const [actioning, setActioning] = useState(false);
    const [sessions, setSessions] = useState<InstituteSessionResponse[] | null>(null);
    ...
    const fetchSessions = useCallback(() => {
        setLoading(true);
        getInstituteSessions({ from, to }).then(setSessions).catch(() => setSessions([])).finally(() => setLoading(false));
    }, [from, to]);
    ...
    const eventsByDate = useMemo(() => {
        ...
        (sessions ?? []).forEach((s) => { (map[s.sessionDate] ??= []).push(s); });
    ...
    const totalEvents = sessions?.length ?? 0;
    const upcomingCount = sessions?.filter(s => s.status === "UPCOMING").length ?? 0;
                                  ...
                                  {formatTime(ev.startTime)} – {formatTime(ev.endTime)}
                                                            ...
                                                            {ev.classroomName && (
                                                                <div className="flex items-center gap-1">
                                                                    ...
                                                                    <span className="text-[11px] text-text-muted truncate">{ev.classroomName}</span>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </motion.button>
                          ...
                          Grade {selectedEvent.grade}
                                ...
                                <div className="p-6 -mt-4">
                                    <div className="bg-white rounded-2xl shadow-sm border border-border p-4 space-y-4">
                                        ...
                                        {selectedEvent.classroomName && (
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-xl bg-danger/10 flex items-center justify-center flex-shrink-0">
                                                    <MapPin className="w-4 h-4 text-rose-500" />
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-semibold text-text-muted uppercase tracking-wider">Classroom</p>
                                                    <p className="text-sm font-semibold text-text">{selectedEvent.classroomName}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
