...
import { getInstituteStudentAttendance } from "@/services/api";
import type { InstituteStudentResponse, InstituteAttendanceRecord } from "@/lib/types";
...
interface Props {
  student: InstituteStudentResponse | null;
  onClose: () => void;
}

export default function InstituteAttendanceDrawer({ student, onClose }: Props) {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const data = await getInstituteStudentAttendance(student.id, params);
      setRecords(data ?? []);
    } catch {
      setRecords([]);
    } finally {
  ...
  useEffect(() => {
    if (student) { setActivePreset("All Time"); setDateFrom(""); setDateTo(""); }
  }, [student]);
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
            ...
            <div className="bg-gradient-to-r from-[#4F46E5] to-[#3730A3] px-6 py-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={onClose} className="p-2 rounded-xl text-white/40 hover:bg-white/10 hover:text-white transition-colors"><X className="w-5 h-5" /></button>
              </div>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                ...
                {records.length > 0 && <><span className="text-white/30">·</span><span className="text-xs text-white/60">{records.length} records</span></>}
              </div>
            </div>
              ...
              return (
                <div className="px-6 py-3 border-b border-slate-100">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs text-slate-500"><span className="font-bold text-slate-800">{pct}%</span> attendance rate</span>
                  ...
                  <div className="flex items-center justify-between mt-1.5">
                    <span className="text-[10px] text-slate-400">{totalPresent} present out of {total}</span>
                    <span className="text-[10px] text-slate-400">{total - totalPresent} missed</span>
                  </div>
                </div>
              );
            })()}
            ...
            <div className="px-6 py-3 border-b border-slate-100">
              <div className="relative">
                <button onClick={() => setDateOpen(!dateOpen)}
                  ...
                  <div className="flex items-center gap-2"><Calendar className="w-4 h-4 text-slate-400" /><span className="text-xs text-slate-600">{activePreset}</span></div>
                      ...
                      <motion.div initial={{ opacity: 0, y: -6, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.97 }} transition={{ duration: 0.15 }}
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {PRESETS.map(p => (
                              <button key={p.label} onClick={() => applyPreset(p)}
                                ...
                                style={activePreset === p.label ? { background: "linear-gradient(135deg, #4F46E5, #3730A3)" } : {}}>{p.label}</button>
                            ))}
                          </div>
                        </div>
                        <div className="p-4 space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">From</label><DatePicker value={dateFrom} onChange={v => { setDateFrom(v); setActivePreset("Custom"); }} fullWidth /></div>
                            <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">To</label><DatePicker value={dateTo} onChange={v => { setDateTo(v); setActivePreset("Custom"); }} minDate={dateFrom} fullWidth /></div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 p-4 border-t border-slate-100 bg-slate-50/40">
                          <button onClick={() => { setDateFrom(""); setDateTo(""); setActivePreset("All Time"); setDateOpen(false); }} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-slate-500 bg-white border border-slate-200 hover:bg-slate-50 transition-all">Reset</button>
                          <button onClick={() => setDateOpen(false)} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white transition-all hover:shadow-md" style={{ background: "linear-gradient(135deg, #4F46E5, #3730A3)" }}>Apply</button>
                        </div>
                      </motion.div>
                    </>
                  )}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {loading ? (
                <div className="space-y-4">{[...Array(5)].map((_, i) => (<div key={i} className="animate-pulse flex gap-3"><div className="w-3 h-3 rounded-full bg-slate-200 mt-1.5" /><div className="flex-1 space-y-2"><div className="h-3 bg-slate-200 rounded w-32" /><div className="h-3 bg-slate-200 rounded w-48" /></div></div>))}</div>
              ) : records.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12">
                  <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center mb-3"><CheckCircle className="w-7 h-7 text-slate-300" /></div>
                        ...
                        <div key={r.id} className="relative pl-7">
                          ...
                          <div className="bg-slate-50 rounded-xl p-3">
                            <div className="flex items-center justify-between mb-1">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${style.bg} ${style.text}`}>{r.status}</span>
                              <span className="text-[10px] text-slate-400">{new Date(r.date).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</span>
                            </div>
                            <p className="text-sm font-bold text-slate-700">{r.className}</p>
                            {r.checkInTime && <p className="text-[10px] text-slate-400 mt-1">Check-in: {new Date(r.checkInTime).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</p>}
                          </div>
                        </div>
                      );
                    })}
